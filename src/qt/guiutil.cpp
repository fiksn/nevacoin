...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("nevacoin"))
        return false;
...
{
    // Convert nevacoin:// to nevacoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("nevacoin://"))
    {
        uri.replace(0, 12, "nevacoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Nevacoin.lnk";
}
...
{
    return GetAutostartDir() / "nevacoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Nevacoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  nevacoin-qt [" + tr("command-line options") + "]                     " + "\n";
